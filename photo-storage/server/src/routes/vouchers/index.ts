...
router.post('/activate', async (req, res) => {
  ...
  if (alreadyUsed) {
    return res.status(400).json({ message: 'Bạn đã sử dụng mã voucher này rồi' })
  }
  ...
  try {
    if (voucher.type === 'plan_activation' && voucher.planId) {
      ...
      if (!plan) return res.status(400).json({ message: 'Gói dịch vụ không tồn tại' })
      ...
      if (currentPlan) {
        ...
        return res.status(400).json({ message: 'Bạn chưa có gói dịch vụ nào để gia hạn' })
      }
    }
    ...
    res.json({ ok: true, type: voucher.type })
  } catch {
    return res.status(500).json({ message: 'Kích hoạt voucher thất bại' })
  }
})
