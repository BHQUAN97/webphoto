...
<template>
  <div>
    ...
    <div class="space-y-3">
      <div v-for="m in methods" :key="m.id" class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex items-center justify-between">
        <div>
          ...
          <pre class="text-xs text-gray-500 mt-1">{{ JSON.stringify(m.config, null, 2).slice(0, 100) }}...</pre>
        </div>
    ...
    <BaseModal :show="showForm" :title="editing ? 'Sửa phương thức' : 'Thêm phương thức'" max-width="lg" @close="showForm = false">
      <form @submit.prevent="save" class="space-y-4">
        ...
        <div><label class="block text-sm font-medium mb-1">Tên hiển thị</label><input v-model="form.name" class="w-full px-3 py-2 border rounded-lg text-sm" /></div>
        <div><label class="block text-sm font-medium mb-1">Config (JSON)</label><textarea v-model="form.configJson" rows="5" class="w-full px-3 py-2 border rounded-lg text-sm font-mono" /></div>
        <div>
          <label class="block text-sm font-medium mb-1">Ảnh QR</label>
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-gray-400 mt-1">JPEG, PNG, WebP. Tối đa 2MB</p>
            </div>
          </div>
        </div>
