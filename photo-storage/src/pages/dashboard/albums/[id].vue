...
function downloadFavoritesList() {
  showDownloadOptions.value = false
  const favImages = images.value.filter(i => i.liked)
  if (favImages.length === 0) {
    toast.error(t('album.noFavorites'))
  ...
  const text = favImages.map(img => img.originalName).join('\n')
  const blob = new Blob([text], { type: 'text/plain;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `${album.value?.title || 'album'}_favorites_list.txt`
  document.body.appendChild(a)
  a.click()
  document.body.removeChild(a)
  URL.revokeObjectURL(url)
  toast.success(t('album.favoriteListDownloaded'))
}
  ...
  <div v-else-if="album">
    ...
    <BaseModal :show="showDownloadOptions" :title="$t('album.downloadOptions')" @close="showDownloadOptions = false">
      <div class="space-y-3">
        ...
        <button
          class="w-full flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-600 hover:border-orange-300 hover:bg-orange-50 dark:hover:bg-orange-900/10 transition-colors text-left"
          @click="downloadFavoritesList"
        >
          <div class="shrink-0 w-9 h-9 rounded-lg bg-pink-50 dark:bg-pink-900/20 flex items-center justify-center">
            <svg class="w-5 h-5 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <div class="min-w-0">
            <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $t('album.downloadFavoritesList') }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $t('album.downloadFavoritesListDesc') }}</div>
          </div>
        </button>
